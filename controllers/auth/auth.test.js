...
describe("Unit test registration", () => {
  ...
  test("SignUp new User", async () => {
    ...
    expect(authService.isUserExist).toHaveBeenCallWith(req.body.email);
    expect(res.status).toHaveBeenCallWith(HttpCode.CREATED);
  });
  ...
  test("SignUp already exist User", async () => {
    ...
    await registration(req, res, next);
    expect(authService.isUserExist).toHaveBeenCallWith(req.body.email);
    expect(res.status).toHaveBeenCallWith(HttpCode.CONFLICT);
  });
  ...
  test("SignUp with error database", async () => {
    ...
    await registration(req, res, next);
    expect(authService.isUserExist).toHaveBeenCallWith(req.body.email);
    expect(next).toHaveBeenCallWith(testError);
  });
});
